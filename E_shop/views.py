...
def clear_wishlist(request):
    if 'wishlist' in request.session:
        del request.session['wishlist']  # Remove wishlist from session
        request.session.modified = True  # Mark session as modified
    return redirect("home")  # Redirect to wishlist page or home
...
def PLACE_ORDER(request):
    if request.method == 'POST':
        ...
        if not postcode.isdigit():
            messages.error(request, "Invalid postcode. Please enter a number.")
    ...
    return render(request,'Cart/placeorder.html', {'order_id':order_id})
